...
async function openProperty(query, pattern) {
  ...
  const result = page.locator('.search-result-row:visible')
  ...
  await page.waitForFunction(({ source, flags }) => {
    ...
    const address = window.PROPERTY_DATA?.canonical_address || '';
    return assessment?.modelVersion === 'LC-BNE-5L-v0.2.1' && new RegExp(source, flags).test(address);
  ...
  await page.waitForFunction(() => Number.isFinite(window.LEMONCHECK_ASSESSMENT?.deal?.score), null, { timeout: 10000 });

  const save = page.locator('[data-save-property]');
  await save.waitFor({ state: 'visible', timeout: 10000 });
  await save.click();
  await page.waitForFunction(() => {
    const items = JSON.parse(localStorage.getItem('lemoncheck-shortlist-v1') || '[]');
    const current = String(window.PROPERTY_DATA?.property_id || '');
    return items.some((item) => String(item.propertyId) === current);
  }, null, { timeout: 10000 });
...
try {
  ...
  if (annie.propertyId === william.propertyId) throw new Error('The second saved property reused the first property ID');

  const compare = page.locator('[data-open-comparison]');
  ...
  const state = await page.evaluate(() => {
    ...
    return {
      ...
      open: Boolean(document.querySelector('#lemoncheck-compare-dialog[open]')),
      scoreGroups: document.querySelectorAll('#lemoncheck-compare-dialog .lc-compare-lenses').length,
    };
  });

  if (state.count !== 2 || state.cards !== 2 || !state.open) throw new Error(`Shortlist comparison did not contain two properties: ${JSON.stringify(state)}`);
  if (!state.text.match(/28\s+Annie\s+Street/i) || !state.text.match(/1\s+William\s+Street/i)) throw new Error(`Saved addresses missing from comparison: ${state.text}`);
  if (state.versions.length !== 1 || state.versions[0] !== 'LC-BNE-5L-v0.2.1') throw new Error(`Comparison mixed model versions: ${JSON.stringify(state.versions)}`);
  if (state.scoreGroups !== 2) throw new Error(`Five-score comparison groups missing: ${state.scoreGroups}`);
  for (const label of ['Lemon', 'Deal', 'Fit', 'Development', 'Confidence']) {
    if (!state.text.includes(label)) throw new Error(`Comparison lens missing: ${label}`);
  }

  const dialogOverflow = await dialog.evaluate((element) => element.scrollWidth - element.clientWidth);
  if (dialogOverflow > 2) throw new Error(`Comparison dialog horizontal overflow: ${dialogOverflow}px`);
